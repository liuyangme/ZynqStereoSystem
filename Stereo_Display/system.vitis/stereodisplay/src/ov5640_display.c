...
#define VDMA0_ID 			XPAR_AXIVDMA_0_DEVICE_ID
...
int main(void)
{
	...
	XAxiVdma InstancePtr0;
	XAxiVdma InstancePtr1;

	xil_printf("Starting the VDMA0 \n\r");
	Status = run_triple_frame_buffer(&InstancePtr0, 0, 1280, 720,
							srcBuffer, 2, 0);
	...
	xil_printf("Starting the VDMA1 \n\r");
	Status = run_triple_frame_buffer(&InstancePtr1, 1, 1280, 720,
							srcBuffer, 2, 0);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Transfer of frames failed with error = %d\r\n",Status);
